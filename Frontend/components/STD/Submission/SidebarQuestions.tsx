...
import { useFetchSubmissionDetails } from "@/hooks/Student/Submission/useFetchSubmissionDetails";
import { useSubmissionDetailsStore } from "@/store/Student/Submission/useSubmissionDetailStore";
import { RubricDetails } from "@/components/STD/Submission/RubricDetails";
import { NoSubmissionDetails } from "@/components/STD/Submission/NoSubmissionDetails";

export const SidebarSubmissionDetails = () => {
  const { course_id, assignment_id, submission_id } = useParams<{ course_id: string; assignment_id: string; submission_id: string }>();
  const [ opened, { toggle } ] = useDisclosure(true);
  const { isLoading: isLoadingSubmissionDetails, isError: isErrorLoadingSubmissionDetails } = useFetchSubmissionDetails({ course_id, assignment_id, submission_id });
  const submissionDetails = useSubmissionDetailsStore((state) => state.submissionDetails);
  const selectedRubricID = useSubmissionDetailsStore((state) => state.selectedRubricID);
  const toggleSelectedRubricID = useSubmissionDetailsStore((state) => state.toggleSelectedRubricID);
