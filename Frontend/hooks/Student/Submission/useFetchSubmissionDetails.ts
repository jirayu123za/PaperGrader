...
export const useFetchSubmissionDetails = ({ course_id, assignment_id, submission_id }: fetchSubmissionDetailsParams) => {
    const { setSubmissionDetails } = useSubmissionDetailsStore(state => ({ setSubmissionDetails: state.setSubmissionDetails }));
    ...
    return useQuery<SubmissionDetails, Error>({
        ...
        queryFn: async (): Promise<SubmissionDetails> => {
            const response = await axios.get(`${API_BASE}/student/submission/details`,
                {
                    params: {
                        course_id: course_id,
